...
iyr:2011 ecl:brn hgt:59in`.split('\n\n');
      const parsedInput = input.map((passport) => passport.split('\n')
        .join(' ')
        .split(/\s+/)
        .map((value) => value.split(':'))
        .reduce((accum, [key, value]) => ({ ...accum, [key]: value }), {}));
      expect(part1(parsedInput).length).toBe(2);
    });

    it('should return the correct value with actual values', () => {
      const input = fs.readFileSync(path.join(__dirname, './input.txt'), { encoding: 'utf-8' }).split('\n\n');
      const parsedInput = input.map((passport) => passport.split('\n')
        .join(' ')
        .split(/\s+/)
        .map((value) => value.split(':'))
        .reduce((accum, [key, value]) => ({ ...accum, [key]: value }), {}));
      expect(part1(parsedInput).length).toBe(228);
    });
  });

  describe('part 2', () => {
    it('should return the zero value with test values', () => {
...
pid:3556412378 byr:2007`.split('\n\n');
      const parsedInput = input.map((passport) => passport.split('\n')
        ...
        .map((value) => value.split(':'))
        .reduce((accum, [key, value]) => ({ ...accum, [key]: value }), {}));
      expect(part2(parsedInput).length).toBe(0);
    });

    it('should return the four value with test values', () => {
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719`.split('\n\n');
      const parsedInput = input.map((passport) => passport.split('\n')
        ...
        .map((value) => value.split(':'))
        .reduce((accum, [key, value]) => ({ ...accum, [key]: value }), {}));
      expect(part2(parsedInput).length).toBe(4);
    });
    ...
    it('should return the correct values with actual values', () => {
      ...
      const parsedInput = input.map((passport) => passport.split('\n')
        ...
        .map((value) => value.split(':'))
        .reduce((accum, [key, value]) => ({ ...accum, [key]: value }), {}));
      expect(part2(parsedInput).length).toBe(175);
    });
  });
});
